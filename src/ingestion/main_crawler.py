...
def __Run(kafka_host: str,
    ...
    tmdb_consumer = XmdbEntryConsumer(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Consumes XMDB entries and updates the corresponding "
                    "ingestion tables with content pulling from external "
                    "sources.")
    parser.add_argument(
        "--kafka_host",
        type=str,
    ...
    parser.add_argument(
        "--postgres_host",
        type=str,
        help="The IP address which points to the postgres ingestion database "
             "server.")
    parser.add_argument(
        "--postgres_password",
        type=str,
